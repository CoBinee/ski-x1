...
// 内部関数
//
static void make_sound(const char *input_path, const char *output_path, const char *header_path, bool verbose);
...
static void save_header(const char *header_path, struct sound_song *song_list, bool verbose);
...
int main(int argc, const char *argv[])
{
    ...
    if (input_path != NULL && output_path != NULL) {
        make_sound(input_path, output_path, header_path, verbose);

    // ヘルプの表示
    } else {
        printf("Convert Famistduio .txt file:\n");
        printf("fmstxt [-v] [-h <header file>] -o <output file> <input_file>\n");
    }
...
static void make_sound(const char *input_path, const char *output_path, const char *header_path, bool verbose)
{
    ...
    // ヘッダの書き出し
    if (header_path != NULL) {
        save_header(header_path, song_list, verbose);
    }
...
static void save_header(const char *header_path, struct sound_song *song_list, bool verbose)
{
    ...
    if (header_file == NULL) {
        fprintf(stderr, "error: save_header() - %s file not opened.\n", header_path);
        exit(1);
    }

    // ヘッダの書き出し
    fprintf(header_file, "; Song List\n;\n\n");
    {
        struct sound_song *song = song_list;
        int index = 0;
        while (song != NULL) {
            size_t name_length = strlen(song->name);
            char *name = new char[name_length + 1];
            for (int i = 0; i < name_length; i++) {
                if (isalnum(song->name[i]) != 0) {
                    name[i] = toupper(song->name[i]);
                } else {
                    name[i] = '_';
                }
            }
            name[name_length] = '\0';
            fprintf(header_file, "    defc    SONG_%-27s    =   %d\n", name, index);
            delete[] name;
            song = song->next;
            ++index;
        }
